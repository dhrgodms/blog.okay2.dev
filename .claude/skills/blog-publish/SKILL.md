...
- `slug` — if missing, derive from the filename: trim, lowercase, spaces → `-`, strip `` /\?%*:|"<> ``. (Astro's glob loader lowercases ids, so the slug must already be lowercase.)
...
- **Wikilinks** `[[Target]]` or `[[Target|Label]]`: check if `Target` matches the slug or title of an existing post in `src/content/blog/*.md`. If it matches, replace with `[Label](/blog/{slug}/)`. Otherwise replace with just the label as plain text.
