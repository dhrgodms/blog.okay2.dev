...
- **Image embeds** `![[name]]`: look for a file named `name` in the same directory as the source md, and in `inbox/`. If found, copy it to `src/assets/blog/{slug}/name` (raw filename on disk) and replace with `![](../../assets/blog/{slug}/{encodeURIComponent(name)})` — URL-encode the filename in the link (e.g. spaces become `%20`), since an unencoded space in a markdown link destination breaks the link. If not found, replace with `<!-- MISSING IMAGE: name -->` and warn the user.
